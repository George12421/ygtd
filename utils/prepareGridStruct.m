...
missingCapacityValue = 3*mean(abs(result.branch(:,14)));
...
for ii = 1:size(capacityVector,1)
    if capacityVector(ii,1) == 0
        ...
        % capacityVector(ii,1) = 1.5*abs(result.branch(ii,14));
        % capacityVector(ii,1) = missingCapacityValue; 
        % 选取均值和初始潮流1.5倍两者之间的最大值
        capacityVector(ii,1) = max(missingCapacityValue, 1.5*abs(result.branch(ii,14)));
    end
end
